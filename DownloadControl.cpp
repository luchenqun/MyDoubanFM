                    ...
                    curl_off_t ultotal, curl_off_t ulnow)
{
    ...
    {
        double speed = 0;
        if(dlnow - d->m_lastTotalSize > 0 && time > 0)
        {
...
bool DownloadFarm::responseCodeHandle(int responseCode)
{
    if(301 == responseCode || 302 == responseCode || 303 == responseCode)
    {
        ...
        if(returnCode == CURLE_OK && newUrl)
        {
            ...
            if(m_redirectCnt < REDIRECT_MAX_COUNT)
            {
                // 重定向次数少于5，获得新的url重新请求
...
//    curl_easy_setopt(m_curl, CURLOPT_FOLLOWLOCATION, 1);// 重定向
    ...
    curl_easy_setopt(m_curl, CURLOPT_XFERINFOFUNCTION, processFunc);   // 进度
...
void DownloadFarm::stop()
{
    ...
    m_stopFlag = true;
    m_redirectCnt = REDIRECT_MAX_COUNT;
    m_retryCnt = RETRY_COUNT;
}
